...
from Pennylane_QNN import flexible_oracle, diffusion_operator, diffusion_less_parameters, diffusion_RX, diffusion_block, compressed_flexible_oracle
...
parser.add_argument('--variation', type=str, default='one_parameter', help="Type of variation")
...
if variation == 'one_parameter_diffusion':
    ...
    def circuit(feature, params):
        ...
        for layer in range(num_layers):
            compressed_flexible_oracle(params[:4])
            diffusion_operator(params[4:])

        return qml.expval(qml.PauliZ(3))
elif variation == '5_params':
    num_params = 5
    @qml.qnode(dev)
    def circuit(feature, params):
        ...
        for layer in range(num_layers):
            compressed_flexible_oracle(params[:4])
            diffusion_less_parameters(params[4])
...
def circuit_training(X_train, Y_train, X_test, Y_test):
    ...
    plt.legend()
    plt.savefig(f'results/Accuracy_{variation}.png')
    plt.close()
    ...
    plt.savefig(f'results/training_loss{variation}.png')
    plt.close()
